...
function buildGraph(allChats, allChatFileNamesAndLengths) {
    ...
    if (allChats.length === 0 || !allChats[0]) {
        return cyElements;
    }
...
function createNode(nodeId, messageId, text, group, allChatFileNamesAndLengths) {
    ...
    if (isBookmark && !allChatFileNamesAndLengths.hasOwnProperty(`${bookmarkName}.jsonl`)) {
        console.info(`Timelines: createNode: 已忽略指向 '${bookmarkName}' 的失效链接；目标聊天文件 '${bookmarkName}.jsonl' 不存在。`);
        isBookmark = false;
...
function groupMessagesByContent(messages) {
    ...
    messages.forEach((messageObj, index) => {
        ...
        try {
            ...
            groups[normalizedMessageText].push({ file_name, index, message });  // `index` is the index of this message object in the original `messages` array.
        } catch (e) {
            console.error(`Timelines: 消息分组失败：${e}: ${JSON.stringify(message, null, 4)}`);
        }
    });
    return groups;
}
