 ...
 * 打开跨会话语义检索结果弹窗。
 *
 * @param {Array<{chatFile: string, messageId: number, name: string, is_user: boolean, preview: string, score: number, tags: Array<string>, bookmark: boolean}>} results
 *   formatGlobalResults 的输出。
...
export function openSemanticGlobalModal(results, context = {}) {
  ...
  const itemsHtml = rows.length
    ? rows
        ...
        .join('')
    : '<div class="semantic-result-empty">未找到可穿越的跨会话结果。</div>';
