...
export function createNode(nodeId, messageId, text, group, allChatFileNamesAndLengths = {}) {
    let bookmark = group.find(({ message }) => {
        ...
        if (message.is_system && typeof message.mes === 'string' && message.mes.includes('Bookmark created! Click here to open the bookmark chat')) return true;
    ...
    // 检查点死链忽略
    if (isBookmark && bookmarkName && !allChatFileNamesAndLengths.hasOwnProperty(`${bookmarkName}.jsonl`) && !allChatFileNamesAndLengths.hasOwnProperty(bookmarkName)) {
        isBookmark = false;
        bookmarkName = null;
    }
 ...
 */
export function highlightPathToRoot(nodeMap, incomingEdgeMap, bookmarkNode, currentHighlightThickness = 4, startingZIndex = 1000) {
