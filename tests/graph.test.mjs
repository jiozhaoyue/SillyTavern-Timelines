...
test('convertToCytoscapeElements orchestrates preprocessing and graph building', () => {
  const history = {
    'chatA.jsonl': [
      { name: 'AI', is_user: false, mes: 'Hello' },
      { name: 'User', is_user: true, mes: 'World' },
    ],
    'chatB.jsonl': [
      { name: 'AI', is_user: false, mes: 'Hello' },
      { name: 'User', is_user: true, mes: 'Tavern' },
    ],
  };

  const elements = convertToCytoscapeElements(history);
  const nodes = elements.filter(e => e.group === 'nodes');
  assert.equal(nodes.length, 4); // root + Hello + World + Tavern
});
