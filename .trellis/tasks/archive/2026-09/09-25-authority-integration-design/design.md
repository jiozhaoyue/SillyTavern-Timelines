...
## 6. 开放问题（2026-09-25 用户已裁定）

1. ✅ **分期路线**：裁定「Phase 0 → Phase 1 连做」——先实机 E2E 验证，随后实施 Phase 1 四件套。
2. ⬜ **A3 默认范围**：未单独裁定，采用本文档保守默认——**默认当前角色，全局为开关**（实施任务验收时如需翻转再议）。
3. ✅ **A4 自动增量索引**：纳入后续计划（默认关不变），不在本轮实施。
4. ✅ **Phase 2（http.fetch embedding 代理）**：确认有需求，纳入后续计划（新增 `http.fetch` 权限），不在本轮实施。同轮确认 **Phase 3（导出服务端留存，+`storage.blob`）** 纳入后续计划。
